import {
  SchedulerConfig as SchedulerInputState,
  Timeslot,
  Assignment,
  ScheduleSolution,
  FailureDiagnostic,
...
export class TimetableSolver {
    ...
    for (const day of this.state.days) {
      ...
      for (let m = start; m + step <= end; m += step) {
        ...
        slot.isBreak = dayBreaks.some(
          (b: any) => !(slotEnd <= b.start || slotStart >= b.end)
        );
  ...
  public init() {
    ...
    this.rooms = [
      ...(this.state.rooms.theoryList ?? []).map((r: Room) => ({ ...r, type: r.type || 'theory' })),
      ...(this.state.rooms.labList ?? []).map((r: Room) => ({ ...r, type: r.type || 'practical' })),
    ];
  ...
  private computeInitialDomain(v: SessionVariable): DomainValue[] {
    ...
    for (const tb of possibleTimeBlocks) {
      ...
      if (v.facultyId) {
        const faculty = this.state.faculties.find((f: Faculty) => f.id === v.facultyId);
  ...
  public getDiagnostics(): FailureDiagnostic[] {
    ...
    for (const vars of infeasibleGroups.values()) {
      ...
      const faculty = this.state.faculties.find((f: Faculty) => f.id === referenceVar.facultyId);
      diagnostics.push({
        subject: sub.name,
        batches: sub.batches,
    ...
    for (const [, vars] of subjectGroups.entries()) {
      const referenceVar = vars[0];
      const sub = referenceVar.subject;
      const faculty = this.state.faculties.find((f: Faculty) => f.id === referenceVar.facultyId);
  ...
  private checkBatchOverloads(v: SessionVariable): boolean {
    for (const batch of v.batches) {
      ...
      for (const [assignedVid] of this.bestAssignments.entries()) {
        const assignedVar = this.variables.find((x) => x.id === assignedVid);
